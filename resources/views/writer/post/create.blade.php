            ...
            <div class="flex w-full h-4/5">
                <form action="{{route('posts.store')}}" method="post" class="w-full h-full flex flex-row-reverse">
                    ...
                    <div class="w-1/2 h-full flex flex-col items-end pr-20 relative">
                        ...
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            <label for="name" class="font-semibold ml-5">: description</label>
                            <textarea name="description" id="description" cols="30" rows="5" class="w-2/5 rounded outline-0 p-2 border border-gray-400">{{old('description')}}</textarea>
                            @error('title')
                            <p class="text-red-700">{{$message}}</p>
                            @enderror
                        </div>
